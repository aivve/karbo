...
Difficulty BlockchainCache::getDifficultyForNextBlock(uint32_t blockIndex) const {
  ...
  auto timestamps = getLastTimestamps(currency.difficultyBlocksCountByBlockVersion(nextBlockMajorVersion), blockIndex+1, skipGenesisBlock);
  auto commulativeDifficulties =
      getLastCumulativeDifficulties(currency.difficultyBlocksCountByBlockVersion(nextBlockMajorVersion), blockIndex+1, skipGenesisBlock);
  return currency.nextDifficulty(nextBlockMajorVersion, blockIndex+1, std::move(timestamps), std::move(commulativeDifficulties));
}
