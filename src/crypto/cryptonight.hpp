...
template<bool SOFT_AES, size_t MEMORY, cryptonight_algo ALGO>
inline void cn_gpu_explode_scratchpad(const uint8_t* input, uint8_t* output)
{
...
void cryptonight_hash_gpu(const void* input, size_t len, void* output, cn_context& ctx0) {
	...
	cn_gpu_explode_scratchpad<SOFT_AES, CRYPTONIGHT_MEMORY, ALGO>(ctx0.hash_state, ctx0.long_state);
